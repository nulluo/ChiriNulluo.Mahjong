...
Namespace HandChecker
    ...
    Public Class ShantenCounter
        ...
        Public Shared TileArray(TileMax - 1) As String

        'UNIMPLEMENTED: もっといいなまえを考案
        Public Shared IndexDictionary As New Dictionary(Of String, Integer)

        Public Const TileMax As Integer = 65
        ...
        Public Shared Function CalculateShanten(handInfo As List(Of String)) As ShantenCountProgress
            ...
            Dim _indexList As List(Of Integer) = handInfo.Select(Function(x) IndexDictionary(x)).ToList
        ...
        Public Shared Function CountMentsuAndMentsCandidate(handInfo As Integer(), level As Integer, progress As ShantenCountProgress) As ShantenCountProgress
            ...
            For j As Integer = 0 To TileMax - 1
                ...
                '' j～j+2の順子がとれるかチェック
                If (j <= TileMax - 1 - 2 AndAlso handInfo(j) >= 1 AndAlso handInfo(j + 1) >= 1 AndAlso handInfo(j + 2) >= 1 AndAlso IsSameSuit(j, j + 2)) Then
                ...
                If (j <= TileMax - 1 - 1 AndAlso handInfo(j) >= 1 AndAlso handInfo(j + 1) >= 1 AndAlso IsSameSuit(j, j + 1)) Then
                    Dim _temp(TileMax - 1) As Integer
                    Array.Copy(handInfo, _temp, handInfo.Length - 1)
                    _temp(j) -= 1
                    _temp(j + 1) -= 1
                ...
                '' 順子候補があればとる（カンチャン）
                '' 九筒+一索など、色をまたいだ牌をカンチャンとしてカウントしないように検証
                If (j <= TileMax - 1 - 2 AndAlso handInfo(j) >= 1 AndAlso handInfo(j + 2) >= 1 AndAlso IsSameSuit(j, j + 2)) Then
                    Dim _temp(TileMax - 1) As Integer
                    Array.Copy(handInfo, _temp, handInfo.Length - 1)
                    _temp(j) -= 1
                    _temp(j + 2) -= 1
        ...
        ''' <summary>
        ''' 2枚の牌IDが全て同じ作品IDに属するかどうかを判定する
        ''' </summary>
        Private Shared Function IsSameSuit(index0 As Integer, index1 As Integer) As Boolean
            Dim _suit0 As String = TileArray(index0).Substring(0, 2)
            Dim _suit1 As String = TileArray(index1).Substring(0, 2)
